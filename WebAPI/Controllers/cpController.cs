...
namespace WebAPI.Controllers
{
    public class CpController : ApiController
    {
        ...
        public DataSet Get()
        {
            var result = DataAccess.DataSet("select cp.*,cp_sort.sort_id as ssort_id from cp left join cp_sort on cp.id=cp_sort.cp_id order by cp.px desc,cp.id desc", this.Url.Request.RequestUri.Segments[1]);
